...
func TestTelnetClient_Send(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.name, func(t *testing.T) {
			...
			// Creating client.
			if tC.name == "nil connection" {
				client = NewTelnetClient("localhost:0", 1*time.Millisecond, in, nil)
			} else {
				client = NewTelnetClient(listener.Addr().String(), 10*time.Second, in, nil)
				if tC.shouldConnect {
...
func TestTelnetClient_Receive(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.name, func(t *testing.T) {
			...
			// Create client.
			if tC.name == "nil connection" {
				client = NewTelnetClient("localhost:0", 1*time.Millisecond, nil, out)
			} else {
				client = NewTelnetClient(listener.Addr().String(), 10*time.Second, nil, out)
				if tC.shouldConnect {
					require.NoError(t, client.Connect(), "connection failed, but shouldn't")
				}
			}
...
func TestTelnetClient_Connect(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.name, func(t *testing.T) {
			...
			client := NewTelnetClient(tC.address, tC.timeout, nil, nil)
			err = client.Connect()
	...
	for _, tC := range testCases {
		t.Run(tC.name, func(t *testing.T) {
			...
			if tC.connected {
				...
				client := NewTelnetClient(listener.Addr().String(), 10*time.Second, nil, nil)
				require.NoError(t, client.Connect(), "Connect() failed")

				if tC.withInput {
					in := io.NopCloser(bytes.NewBuffer(nil))
					clientWithIn := NewTelnetClient(listener.Addr().String(), 10*time.Second, in, nil)
				...
				client := NewTelnetClient("localhost:0", 1*time.Second, nil, nil)
				err = client.Close()
			}
