...
- Добавлены Dockerfiles для сборки сервисов в контейнеры (`./build/`)
- Дописан `docker-compose.yaml`:
  - Добавлены сервисы проекта
  - Дополнительно поднимается контейнер, накатывающий миграции. Стартует только при чистом запуске - при обычном перезапуске контейнеров останется том, защищающий сервисы от повторного применения миграций
  - Дополнительно поднимается контейнер, ожидающий результат применения миграций. Контейнеры календаря ожидают его завершения

**Интеграционные тесты:**

- Основные ручки сервиса, указанные в ТЗ, тестируются с помощью Go тестов (`./test/integration/integration_test.go`)
- В том же файле реализованы тесты, проверяющие подчистку БД сервисом `scheduler`
- Тесты на логику уведомлений реализованы в виде отдельного shell-скрипта (`./test/integration/sender_test.go`)
- Для тестов собирается отдельное изолированное окружение:
  - Своя сеть
  - Свои тома
  - Свои контейнеры с сервисами, БД и Rabbitom
- Для тестовой сборки все конфиги (как сервисов, так и Docker/docker compose) вынесены отдельно, чтобы можно было настраивать оптимальные для тестов параметры

**Makefile:**

- Добавлены команды `make up`, `make down` и `make integration-tests`
- `make integration-tests` работает в несколько этапов:
  - Поднимает тестовое окружение
  - Собирает результаты Go тестов
  - Запускает скрипт для тестирования уведомлений
  - Собирает его результаты
  - Определяет, все ли тесты пройдены

---

## Дополнительные сервисы и RabbitMQ
